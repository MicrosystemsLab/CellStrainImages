...
% IMREGOFEACHCELL(FNS,CELLSIZE) estimates strain in each cell by comparing it to the
...
cellOrnt = zeros(length(ind),size(FNS,2));
% get cell properties for "baseline average" image
cellProp = regionprops(waterIm,'Eccentricity','Perimeter','Area','Orientation');
...
cellOrnt(:,1) = cat(1,cellProp.Orientation);
...
for i = 2:size(FNS,2)
	...
	for j = 1:length(ind)
		...
		scp = regionprops(selectedCellStrain,'Eccentricity','Perimeter','Area','Orientation');
		cellArea(j,i) = scp.Area;
		cellPerm(j,i) = scp.Perimeter;
		cellEccn(j,i) = scp.Eccentricity;
		cellOrnt(j,i) = scp.Orientation;
...
save('cellProperties.mat','cellArea','cellPerm','cellEccn','cellOrnt');
